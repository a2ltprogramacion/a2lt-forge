...
```bash
python .agent/skills/ghl-list-ai-agents/scripts/fetch_ai_agents.py
```
...
1. Auto-generates `.agent/.env` template (if `.agent/` exists, else `.env` in root)
...
- `agent-creator-pro` (Paso 0: Discovery) — loads existing agents for orchestration planning
- `brainstorming` (design phase) — maps skills to available agents

**Downstream (Uses outputs of this skill):**
- `rag-indexer` (indexes agent metadata for later queries)
- Custom orchestration workflows (delegation logic)
